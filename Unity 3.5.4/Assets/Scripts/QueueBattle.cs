...
public class QueueBattle : MonoBehaviour {
	...
	public void showLoadoutCard(int playerId, Rect Position, int Faction) {
		...
		// Set the current player to the player in the room matching its player ID
		if (currentActiveRoom != null) {
			foreach (User userInRoom in currentActiveRoom.UserList) {
				if (playerId == userInRoom.PlayerId) {
					user = userInRoom;
					break;
				}
			}
