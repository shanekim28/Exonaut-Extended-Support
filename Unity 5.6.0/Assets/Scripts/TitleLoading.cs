...
public class TitleLoading : MonoBehaviour
{
	...
	private IEnumerator WaitForPickupRequest(WWW www)
	{
		...
		if (www.error == null)
		{
			...
			{
				AssetBundleRequest abr = assetBundle.LoadAssetAsync(str, typeof(GameObject));
				yield return abr;
	...
	private IEnumerator WaitForWeaponsRequest(WWW www)
	{
		...
		if (www.error == null)
		{
			yield return www;
			AssetBundle assetBundle = www.assetBundle;
			UnityEngine.Object[] weaponsArray = assetBundle.LoadAllAssets(typeof(GameObject));
	...
	private IEnumerator WaitForAnimationRequest(WWW www)
	{
		...
		AssetBundle assetBundle = www.assetBundle;
		UnityEngine.Object[] anims = assetBundle.LoadAllAssets(typeof(AnimationClip));
	...
	private IEnumerator WaitForLowSuitsRequest(WWW www)
	{
		...
		if (www.error == null)
		{
			...
			AssetBundle assetBundle = www.assetBundle;
			UnityEngine.Object[] suits = assetBundle.LoadAllAssets(typeof(GameObject));
			for (int i = 0; i < suits.Length; i++)
			{
				foreach (Exosuit Suit in GameData.MasterSuitList.Values)
				{
					if (suits[i].name.Contains(Suit.mSuitFileName))
					{
						...
						AssetBundleRequest abr = assetBundle.LoadAssetAsync(textureName, typeof(Material));
						yield return abr;
	...
	private IEnumerator WaitForEventBundleRequest(WWW www)
	{
		...
		if (www.error == null)
		{
			...
			AssetBundleRequest abr = assetBundle.LoadAssetAsync("event_manifest", typeof(TextAsset));
			yield return abr;
			if (abr != null)
			{
			...
			foreach (string events in array)
			{
				...
				{
					GameObject go = assetBundle.LoadAsset(val3, typeof(GameObject)) as GameObject;
					if (go == null)
					{
				...
				{
					Texture2D t2D = assetBundle.LoadAsset(val3, typeof(Texture2D)) as Texture2D;
				...
				{
					AudioClip Clip = assetBundle.LoadAsset(val3, typeof(AudioClip)) as AudioClip;
					if (Clip == null)
					{
				...
				{
					TextAsset Text = assetBundle.LoadAsset(val3, typeof(TextAsset)) as TextAsset;
